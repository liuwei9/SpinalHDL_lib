...
class xpm_fifo_async(
    ...
    addGeneric("CDC_SYNC_STAGES", CDC_SYNC_STAGES)
    ...
    addGeneric("RELATED_CLOCKS", RELATED_CLOCKS)
    ...
    noIoPrefix()
    mapClockDomain(wr_clk ,io.wr_clk, reset = io.rst)
    mapClockDomain(rd_clk , io.rd_clk)
}

object xpm_fifo_async {
    def apply(
                 CDC_SYNC_STAGES: Int = 2,
                 DOUT_RESET_VALUE: String = "0", // String
                 ECC_MODE: String = "no_ecc", // String
               ...
               din: Bits, injectdbiterr: Bool, injectsbiterr: Bool, rd_en: Bool,  sleep: Bool, wr_en: Bool): xpm_fifo_async ={
        val temp = new xpm_fifo_async(CDC_SYNC_STAGES,DOUT_RESET_VALUE, ECC_MODE, FIFO_MEMORY_TYPE, FIFO_READ_LATENCY, FIFO_WRITE_DEPTH, FULL_RESET_VALUE, PROG_EMPTY_THRESH,
            PROG_FULL_THRESH, RD_DATA_COUNT_WIDTH, READ_DATA_WIDTH, READ_MODE, RELATED_CLOCKS,SIM_ASSERT_CHK, USE_ADV_FEATURES, WAKEUP_TIME, WRITE_DATA_WIDTH, WR_DATA_COUNT_WIDTH,wr_clk,rd_clk)
...
class fifo_async (
    ...
    val sleep=Bool()
    sleep:= False
    val temp = xpm_fifo_async(CDC_SYNC_STAGES,DOUT_RESET_VALUE, ECC_MODE, FIFO_MEMORY_TYPE, FIFO_READ_LATENCY, FIFO_WRITE_DEPTH, FULL_RESET_VALUE, PROG_EMPTY_THRESH, PROG_FULL_THRESH,
        RD_DATA_COUNT_WIDTH, READ_DATA_WIDTH, READ_MODE_A, RELATED_CLOCKS,SIM_ASSERT_CHK, USE_ADV_FEATURES, WAKEUP_TIME, WRITE_DATA_WIDTH, WR_DATA_COUNT_WIDTH,wr_clk,rd_clk)(
...
object fifo_async {
    def main(args: Array[String]): Unit = {
        SpinalConfig(targetDirectory = "verilog/xfifo").generateVerilog( new fifo_async(12, 1024, 12,0,"block","fwft"))
    }
}
